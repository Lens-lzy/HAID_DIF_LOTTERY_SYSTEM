...
/************ 基本配置 ************/
const HOST_IP     = '8.148.238.85'; // 日志展示用
...
staffApp.post('/api/startDraw', auth('staff'), (req,res)=>{
  ...
  const isVIP = String(empId).trim() === '1318601';
...
const STAFF_PORT  = Number(process.env.STAFF_PORT) || 3000;
// 单端口：直接让 ADMIN_PORT 等于 STAFF_PORT
const ADMIN_PORT  = STAFF_PORT;
const PUBLIC_HOST = process.env.PUBLIC_HOST || '8.148.238.85';

/************ 启动并输出可点击地址 ************/
const staffUrl = `http://${PUBLIC_HOST}:${STAFF_PORT}/staff.html`;
const adminUrl = `http://${PUBLIC_HOST}:${ADMIN_PORT}/admin.html`;

staffServer.listen(STAFF_PORT, HOST, () => {
  console.log(`👷 Staff  : ${staffUrl}`);
